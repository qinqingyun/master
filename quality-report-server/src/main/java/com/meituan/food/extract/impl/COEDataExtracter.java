...
public class COEDataExtracter implements ICOEDataExtract {
    ...
    public void getCOEData(String firstDateStr, String secondDateStr) throws ParseException {
        ...
        /*
        到店餐饮研发部且不包含到店餐饮研发部/餐饮解决方案组的数据
        包含平台业务研发部/商家平台研发组的数据
        包含平台业务研发部/客户平台研发组的数据
        包含平台业务研发部 /供应链平台研发组 / 到餐研发组的数据
        包含平台业务研发部 /销售支持平台研发组 / 到餐研发组的数据
        包含平台业务研发部 /交易平台研发组 / 到餐研发组的数据
        包含平台业务研发部 /营销平台研发组 / 到餐研发组的数据
        包含平台业务研发部/平台终端研发组/到餐业务研发组的数据
        包含测试部/到店餐饮测试组的数据
         */
