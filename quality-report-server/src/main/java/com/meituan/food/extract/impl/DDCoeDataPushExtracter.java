...
public class DDCoeDataPushExtracter implements ICargoDataPushExtract {
    ...
    public void pushData() throws ParseException {
        ...
        String modifyDate = "2020-11-24 00:00:00";
        ...
        for (McdCoePO mcdCoePO : mcdCoePOList) {
            if (!(pushList.keySet().contains(mcdCoePO.getOwnerMis()))) {
                //过滤 2020-11-24之前小黄卡的规范巡检
                if (mcdCoePO.getOrgName().contains("小黄卡研发组") && mcdCoePO.getCreateTime().before(sdf.parse(modifyDate))) {
                    continue;
                }
        ...
        for (McdCoePO po : threeDayCoePoiList) {
            if (!(remindPushList.keySet().contains(po.getOwnerMis()))) {
                //过滤 2020-11-24之前小黄卡的规范巡检
                if (po.getOrgName().contains("小黄卡研发组") && po.getCreateTime().before(sdf.parse(modifyDate))) {
                    continue;
                }
    ...
    public void getPushContext(Map<String, String> pushList, McdCoePO mcdCoePO, int index, String content) throws ParseException {
        ...
        if (index != 0) {
            content = content + "\n" + "[点此处理|" + mcdCoePO.getCoeLink() + "]        [COE书写规范|https://km.sankuai.com/page/192873360]   （如有疑问请大象联系张洋洋(zhangyangyang17)）";
