 ...
 * 数据源:       shopId：24799161
 * 主要流程:     下单 -> 支付 -> 详情 -> 退款
 * 备注:        平台：美团小程序 ；买单方案：全单折买单；退款方式：直接退款
 **/

@ClassAnnotation(author = "byq", depart = "C", des = "test环境小程序美团下单流程")
@Slf4j
public class TestmtWeixinDiscountScence extends TestDPLogin {
    ...
    public void ms_c_mTWeixinoriginalScenes() throws Exception {
        ...
        //1、加载优惠台
        int couponOfferId = CreateOrderUtil.wxMtaloadUnicashier(mtToken, dpWxClient, "ms_c_mt_wxaloadcashier_01");
        log.info("折扣couponOfferId:" + couponOfferId);
        //1、输入金额创建订单（下单支付）
        Map<String, String> wxaOrderCreateResult = CreateOrderUtil.wxaMtCreateOrder(mtToken,"ms_c_mt_wxDiscountCreateOrder_01", couponOfferId, null,null);
        ...
        //Assert.assertEquals(jsonObject.getString("success"),"true","操作退款失败");
