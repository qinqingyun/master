...
public interface JenkinsViewPOMapper {
   ...
   @Select({
            "select",
            "url,view",
            "from ( select url,view,concat('meishi_b/',job) as job from( select url,job, view from jenkins_views  where status=1 and view in ('C端test环境覆盖率','基础测试组','运营测试组','增值测试组','C端test环境覆盖率计算','M端-CRM代理商','M端-MOMA','供应链自动化','结算自动化','客户平台-Test环境接口自动化','TDC门店信息') group by job  order by view )a) b  left outer join ( select dc_job_name,depart_name from dc_case_issue  where build_date>=#{build_date,jdbcType=VARCHAR} group by dc_job_name order by view_name) c on b.job COLLATE utf8mb4_unicode_ci = c.dc_job_name COLLATE utf8mb4_unicode_ci" ,
            "where c.dc_job_name is NULL and job not in ('meishi_b/SupplyChainSelfHelp-Workflow','meishi_b/MerchantFinancePlatWorkflow','meishi_b/MerchantFinanceWorkflow')"
    })
    @Results({
            @Result(column="url", property="url", jdbcType=JdbcType.VARCHAR),
            @Result(column="view", property="view", jdbcType=JdbcType.VARCHAR)
